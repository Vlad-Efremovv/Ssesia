...
int main()
{
	...
	cout << "Вы хотите создать файл?(Да(1)/Нет(0))\n"
		<< "Введите значение: ";
	bool temp;
	cin >> temp;
	if (!temp)
	{
		cout << "Вы отказались от создания файла!\n";
		cin.get();
		return 0;
	}
	...
	hFile = CreateFileA((LPCTSTR)path.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
